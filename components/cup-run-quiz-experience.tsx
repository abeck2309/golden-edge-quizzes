...
function AnswerHeroGraphic({
  ...
  activeQuestion: TypedQuizQuestion | null;
  eyebrow: string;
  imageOnly?: boolean;
}) {
  const visibleImages = activeQuestion?.answerImages?.slice(0, 2) ?? [];
  const label = activeQuestion?.displayAnswer;
  ...
  return (
    <section className="relative min-h-[360px] overflow-hidden rounded-2xl border border-white/10 bg-[#0c1015] lg:sticky lg:top-8 lg:min-h-[640px]">
      ...
      {visibleImages.length > 0 || !activeQuestion ? (
        <>
          <div
          ...
          {!imageOnly ? (
            <div className="absolute inset-x-0 bottom-0 p-6 md:p-8">
              <p className="eyebrow">{activeQuestion ? "Current Question" : eyebrow}</p>
              <h2 className="mt-4 font-[family-name:var(--font-heading)] text-4xl font-bold tracking-tight text-white md:text-5xl">
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="mx-auto grid w-full max-w-7xl gap-8 px-5 pb-20 pt-8 md:px-8 md:pt-12 lg:grid-cols-[0.74fr_1.26fr] lg:items-start">
        <AnswerHeroGraphic activeQuestion={activeQuestion} eyebrow={eyebrow} imageOnly={heroImageOnly} />
        <TypedQuiz
          title={title}
